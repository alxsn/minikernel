...
//Inicializa a fila de prontos e seu mutex exclusivo
...
void queue_push_back(Queue *q, PCB *pcb){       
    ...
    //trava o mutex da fila de prontos
    pthread_mutex_lock(&q->mutex);

    //verifica se ja existem elementos na fila
    if(q->size == 0){
        q->head = node;
        q->tail = node;
    }
    ...
    //destrava o mutex da fila de prontos
